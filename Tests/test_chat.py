...
def test_chat():
    ...
    homepage(user2.username, client1)
    homepage(user.username, client2)
    ...
    assert rv.status_code == 200
    assert render_template("chat.html", messages=history, user=user2.username).encode('utf-8') in rv.data
